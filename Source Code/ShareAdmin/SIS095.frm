...
Begin VB.Form frmSIS095 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Update Stocks with Split "
   ClientHeight    =   3765
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6735
...
If rsMain.RecordCount < 0 Then
 ...
 With rsMain
  .MoveFirst
  If Not .EOF Then
    rsCert.Open "CERTMST", cnn, adOpenDynamic, adLockPessimistic, adCmdTable
    iOpenCert = True
    '---
    BonusUpdate
  '-- update nextcert on company record
   rsComp!Totstocks = rsComp!Totstocks - iTotBon
   rsComp!issStocks = iTotBon
   rsComp.Update
...
'---------------------------
iErr = 181
i = csvYesNo(iErr, "Update Stock Splits Stocks")
If i = False Then
    OpenErr = True
    cmdBtn_Click (0)
    Exit Sub
...
'-- open BONREF table --
'-----------------------
rsMain.Open "STKBONUS", cnn, , , adCmdTable
iOpenMain = True
If rsMain.EOF = True Then
     iErr = 180
     csvShowUsrErr iErr, "Calculate Stock Splits"
     rsMain.Close
     iOpenMain = False
...
iOpenAct = True
'--
'-- clear all certs from CERTMST
sql = "DELETE from CERTMST"
i = csvADODML(sql, cnn)
'--
OpenFiles_Exit:
   Exit Sub
...
With rsMain
 ...
 While Not .EOF
   ...
   '-- add to stkactiv for printing
    ...
    rsAct!TRNDATE = Now
    rsAct!TrnCode = "X"  'Stock Splits Issues
    rsAct!Status = "O"
    rsAct!FRCERT = 0
   ...
   '-- Add the Stock Splits cert
    ...
    rsCert!Status = "A"
    rsCert!assigned = 0
    rsCert!TrnBatch = "SPLIT"
    rsCert!TRNDATE = Now
    rsCert!Remarks = "Stock Splits Issue"
    rsCert.Update
    '-- Update stkname
    sql = "Update stkname set shares = " & !allocated _
          & " where clientid = " & !ClientId
    X = csvADODML(sql, cnn)
    iTotBon = iTotBon + !allocated
    '-- count total Stock Splits applied
    iCommit = iCommit + 1
